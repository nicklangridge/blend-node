...
_.assign(Feed.prototype, {
  fetch: function(url) {
    var self = this;
    return this.parseFeed(url).then(function(reviews) {
      return Promise.resolve(
        reviews.map    (self.parseReview,  self)
               .filter (self.validReview,  self)
               .filter (self.wantReview,   self)
               .map    (self.expandReview, self)
      );
    });
  },

  parseFeed: function(url) {
    return feedRead(url).then(function(articles) {
